...
export const getAllProducts = catchASync(async (req, res) => {
  ...
  if (category && !mongoose.Types.ObjectId.isValid(category))
    throw new AppError(400, "invalid cateogry input");
  ...
  if (deleteImages) {
    ...
    await Promise.all(
      deleteImages.map(async (image) => {
        ...
        images = images.filter((i) => String(i._id) !== String(image._id));
      })
    );
  }
...
export const deleteSpecificProducts = catchASync(async (req, res) => {
  ...
  const products = await Product.find({ _id: { $in: req.body.ids } }).select(
    "_id images"
  );
