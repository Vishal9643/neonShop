  ...
  return (
    <div className={className || "sticky top-0 w-full z-20"}>
      <div className="bg-black text-white shadow">
        ...
        <div className="logo"> <img src="/img/logo1.png" width={200} height={75}/> 
          ...
          <div className="hidden lg:flex items-center gap-7 capitalize text-lg">
            ...
            />
            <div ref={dropDownRef} className="hover-active">
              <DropDown
                containerRef={dropDownRef}
                title="shop neons"
              />
            </div>
            <CustomLink href="/about" text="about" className="hover-active" />
            <CustomLink
              href="/contact"
              text="contact"
              className="hover-active"
...
const CartBtn = () => {
  const [globalState, setGlobalState] = useGlobalContext();
  const numOfItems =
    globalState.cartData.cart?.items?.length +
    globalState.cartData.cart?.customItems?.length;
  return (
    <button
      className="relative"
      onClick={() =>
