  ...
  const discountPrice =
    product.salePercentage > 0
      ? product.sizes[0].price -
        (product.sizes[0].price * product.salePercentage) / 100
      : product.sizes[0].price;
  return (
    <div className="flex-1 flex flex-col items-center gap-5 text-black relative shadow-2xl border border-gray-200 pb-2">
      ...
      <CustomLink
        href={`/shop/${product.category.slug}/${product.slug}`}
        text="buy now"
        className="px-5 sm:px-10 sm:py-3 md:px-16 flex items-center justify-center bg-black text-white uppercase rounded hover-active"
      />
    </div>
  );
};
