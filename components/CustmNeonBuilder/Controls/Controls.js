...
import {
  ...
  ROUND,
  SQUARE,
  useNeonBuilderContext,
} from "../../../context/NeonBuilderContext";
  ...
  const addToCart = () =>
    catchAsync(
      async () => {
        ...
        if (
          globalState.cartData.cart?.items ||
          globalState.cartData.cart?.customItems
        ) {
        ...
        setGlobalState((state) => ({
          ...state,
          cartData: { ...state.cartData, cart: res.data.cart, show: true },
        }));
      },
      setGlobalState,
      () => setLoading(false)
              ...
              {colors.map((color, i) => (
                ...
                >
                  ...
                  {color.name !== "multiple color" && (
                    <input
                      // value="#FF0000"
                      className="color-item w-6 rounded-3xl cursor-pointer text-transparent"
                      style={{
                        background: `rgb(${color.r}, ${color.g}, ${color.b})`,
                      }}
                    />
    ...
    <button
      ...
      onClick={() =>
        setState((state) => ({
          ...
          data: {
            ...state.data,
            backing: { ...state.data.backing, backingColor: color },
          },
        }))
      }
    >
    ...
    <button
      ...
      onClick={() =>
        setState((state) => ({
          ...
          data: {
            ...state.data,
            backing: { ...state.data.backing, backingType: type },
          },
        }))
      }
    >
